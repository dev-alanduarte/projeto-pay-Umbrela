...
app.use(helmet({
  contentSecurityPolicy: {
    directives: {
      ...
      connectSrc: ["'self'", "http://localhost:3000", "http://localhost:3001", "http://127.0.0.1:3000", "http://127.0.0.1:3001", "ws://localhost:3001", "ws://127.0.0.1:3001", "wss://pagamentoseguromarketplace.com", "https://pagamentoseguromarketplace.com", "https://cdn.socket.io"],
    },
  },
}));
app.use(cors({
    ...
    try {
      ...
      if (error.code === 'ECONNABORTED' || error.message.includes('timeout') || error.code === 'ETIMEDOUT') {
        return res.status(504).json({
          success: false,
          error: 'Timeout ao conectar com a API UmbrellaPag. Tente novamente em alguns instantes.',
          details: {
            code: error.code,
